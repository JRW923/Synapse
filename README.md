...
**本机一体化（推荐）**：一条命令同时启动网页服务和本机 Connector，自动完成配对、
无需第二个终端、日志干净，并自动打开浏览器。

```bash
# 先用 synapse 完成一次本机模型配置，然后：
synapse web --workspace ~/src/my-project
```

该命令在本进程内启动 HTTP 服务并自动把本机工作区绑定到网页，网页端会直接认领、
无需手动点击「连接本机」或输入配对码；`Ctrl+C` 一次即干净退出。

**分离部署（公网 / 多用户）**：服务和 Connector 可分别运行在不同机器。
